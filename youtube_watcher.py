...
def fetch_videos(google_api_key, youtube_playlist_id, page_token= None):
	...
	next_item_token = payload.get('nextPageToken')
	if next_item_token is not None:
		yield from fetch_videos(google_api_key, youtube_playlist_id, next_item_token)
...
def fetch_playlist_items(google_api_key, youtube_playlist_id, page_token= None):
	...
	next_item_token = payload.get('nextPageToken')
	if next_item_token is not None:
		yield from fetch_playlist_items(google_api_key, youtube_playlist_id, next_item_token)
...
def main():
	...
	kafka_config = config['kafka'] | {
		...
		'value.serializer':AvroSerializer(schema_registry_client, youtube_videos_values_schema.schema.schema_str)
	}
	...
	while True:
		...
		for video_item in fetch_playlist_items(google_api_key,youtube_playlist_id):
			...
			for video in fetch_videos(google_api_key,video_id):
				logging.info("GOT %s", pformat(summarize(video)))
				producer.produce(
